...
public class StageSelectManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // --- キーボード入力 ---
        bool movedRight = Input.GetKeyDown(KeyCode.D) || Input.GetKeyDown(KeyCode.RightArrow);
        bool movedLeft = Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.LeftArrow);
        ...
        if (Gamepad.current != null)
        {
            ...
            if (!movedRight)
                movedRight = dpadRight || (stickTriggered && stickRight);
            if (!movedLeft)
                movedLeft = dpadLeft || (stickTriggered && !stickRight);
